...
static void ksu_try_umount_path(const char *mnt, int flags)
{
	struct path path;
	int err;
...
void susfs_try_umount(uid_t target_uid) {
	...
	list_for_each_entry_reverse(cursor, &LH_TRY_UMOUNT_PATH, list) {
		if (cursor->info.mnt_mode == TRY_UMOUNT_DEFAULT) {
			ksu_try_umount_path(cursor->info.target_pathname, 0);
		} else if (cursor->info.mnt_mode == TRY_UMOUNT_DETACH) {
			ksu_try_umount_path(cursor->info.target_pathname, MNT_DETACH);
		} else {
...
static int susfs_prctl_handler(struct kprobe *p, struct pt_regs *regs)
{
	...
	{
		bool enabled = false;
		if (copy_from_user(&enabled, (void __user *)arg3, sizeof(enabled)))
			error = -EFAULT;
		else
	...
	case CMD_SUSFS_SHOW_VERSION:
		if (arg3) {
			if (copy_to_user((void __user *)arg3, SUSFS_VERSION, strlen(SUSFS_VERSION) + 1))
				error = -EFAULT;
		}
		break;
...
#endif
		if (arg3) {
			if (copy_to_user((void __user *)arg3, &features, sizeof(features)))
				error = -EFAULT;
		}
		break;
	}
	case CMD_SUSFS_SHOW_VARIANT:
		if (arg3) {
			if (copy_to_user((void __user *)arg3, SUSFS_VARIANT, strlen(SUSFS_VARIANT) + 1))
				error = -EFAULT;
		}
		break;
	default:
...
static struct kprobe prctl_kp = {
	...
	.pre_handler = susfs_prctl_handler,
};
...
void susfs_init(void) {
	...
	if (rc)
		pr_warn("susfs: prctl kprobe failed: %d\n", rc);
	else
		pr_info("susfs: prctl kprobe registered\n");
	pr_info("susfs: initialized! version: %s variant: %s\n", SUSFS_VERSION, SUSFS_VARIANT);
}
